...
Use the `Task` tool with `subagent_type` and `model` parameters.
...
|-------|------|--------|-------------|
| ARCHITECT | Requirements → step docs | Thorough | `subagent_type: Plan, model: opus` |
| WORKER | Implements steps | Follow spec exactly | `subagent_type: general-purpose, model: opus` |
| VALIDATOR | Reviews against requirements | **HOSTILE - default REJECT** | `subagent_type: general-purpose, model: opus` |
| FINAL VALIDATOR | Reviews ALL changes together | **HOSTILE - catches cross-step issues** | `subagent_type: general-purpose, model: opus` |
| DOCUMENTARIAN | Updates `docs/architecture` | Accurate | `subagent_type: general-purpose, model: opus` |

### Parallel Step Execution
For independent steps, launch multiple Task agents in parallel using `run_in_background: true`:
```
...
│                                                                 │
│   WORKER: Task(subagent_type: general-purpose, model: opus)     │
│      → Implement → $WORKDIR/step_N_impl.md                      │
│      ↓                                                          │
│   BUILD CHECK (output to $WORKDIR/logs/build_stepN_iterM.log)   │
│      ↓                                                          │
│   VALIDATOR: Task(subagent_type: general-purpose, model: opus)  │
│      → Review → $WORKDIR/step_N_valid.md                        │
│      ↓                                                          │
│   PASS → next step    REJECT → iterate (max 5)                  │
│                                                                 │
│ PARALLEL EXECUTION: For independent steps, launch multiple      │
│ Task agents with run_in_background: true in a single message    │
│                                                                 │
│ DO NOT STOP BETWEEN STEPS - continue automatically              │
└─────────────────────────────────────────────────────────────────┘
```

**Background Parallel Execution (for independent steps):**
```
# Launch independent steps in parallel (single message with multiple Task calls)
Task(subagent_type: general-purpose, model: opus, run_in_background: true)
  prompt: "Implement Step 1: ..."
Task(subagent_type: general-purpose, model: opus, run_in_background: true)
  prompt: "Implement Step 2: ..." (if Deps: [none])

# Check progress
TaskOutput(task_id: step1_id, block: false)
TaskOutput(task_id: step2_id, block: false)
```
...
- Background agents: Use `run_in_background: true` to isolate agent context
...
2. Use `TaskOutput(task_id)` to retrieve background agent results
