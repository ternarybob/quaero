# 3agents-tdd - TDD Enforcement Workflow

**Execute:** $ARGUMENTS

**TDD enforcement - tests are IMMUTABLE, fix code until tests pass. Sequential execution with full restart on fix. Output captured to files to prevent context overflow.**

**Read first:** `.codebuff/skills/refactoring/SKILL.md`
...
DATE=$(date +%Y-%m-%d)
TIME=$(date +%H%M)
WORKDIR=".codebuff/workdir/${DATE}-${TIME}-tdd-${TASK_SLUG}"
mkdir -p "$WORKDIR"
...
- `.codebuff/skills/refactoring/SKILL.md` - Core patterns
- `.codebuff/skills/go/SKILL.md` - Go changes
- `.codebuff/skills/frontend/SKILL.md` - Frontend changes
- `.codebuff/skills/test-architecture/SKILL.md` - Test patterns (MANDATORY for API tests)

**Step 1.4: Read test architecture**
- `docs/architecture/TEST_ARCHITECTURE.md`

**Step 1.5: For market worker tests**
- Validate against skill: `.codebuff/skills/test-architecture/SKILL.md`
...
All test output MUST go to file. Only see pass/fail + brief error summary.
...
For tests in `test/api/market_workers/` or `test/api/portfolio/`, verify test outputs per `.codebuff/skills/test-architecture/SKILL.md`:
...
if [ -n "$LATEST_RESULTS" ] && [ -d "$LATEST_RESULTS" ]; then
    ...
    if [ $TOTAL_ISSUES -gt 0 ]; then
        ...
        echo "│ 3. See: .codebuff/skills/test-architecture/SKILL.md             │"
        echo "│                                                                 │"
        ...
        echo "Test must use TestOutputGuard pattern per .codebuff/skills/test-architecture/SKILL.md" >> "$WORKDIR/test_issues.md"
    else
        echo ""
...
# CORRECT: Build output to file, summary only
...
# WRONG: Direct output (will overflow context)
go test -v ./$TEST_PKG/...
...
- Log files in `logs/` provide full debugging context without bloating context
- This provides complete traceability from test results back to TDD session

## INVOKE
```
/3agents-tdd test/ui/job_definition_test.go
# → .codebuff/workdir/2024-12-17-1430-tdd-job_definition/
...
**This workflow runs with tests as IMMUTABLE LAW and all output captured to files.**
