...
**Read first:** `.opencode/skills/refactoring/SKILL.md`
...
DATE=$(date +%Y-%m-%d)
TIME=$(date +%H%M)
WORKDIR=".opencode/workdir/${DATE}-${TIME}-tdd-${TASK_SLUG}"
mkdir -p "$WORKDIR"
...
- Background agents: Use `run_in_background: true` if running parallel analysis (if supported)
...
- `.opencode/skills/refactoring/SKILL.md` - Core patterns
- `.opencode/skills/go/SKILL.md` - Go changes
- `.opencode/skills/frontend/SKILL.md` - Frontend changes
- `.opencode/skills/monitoring/SKILL.md` - UI tests
- `.opencode/skills/test-architecture/SKILL.md` - Market worker tests (MANDATORY for `test/api/market_workers/`)

**Step 1.4: Read test architecture**
- `docs/TEST_ARCHITECTURE.md`

**Step 1.5: For market worker tests**
- Validate against skill: `.opencode/skills/test-architecture/SKILL.md`
...
All test output MUST go to file. Agent only sees pass/fail + brief error summary.
...
For tests in `test/api/market_workers/` or `test/api/portfolio/`, verify test outputs per `.opencode/skills/test-architecture/SKILL.md`:
...
if [ -n "$LATEST_RESULTS" ] && [ -d "$LATEST_RESULTS" ]; then
    ...
    if [ $TOTAL_ISSUES -gt 0 ]; then
        ...
        echo "│ 3. See: .opencode/skills/test-architecture/SKILL.md               │"
        echo "│                                                                 │"
        ...
        echo "Test must use TestOutputGuard pattern per .opencode/skills/test-architecture/SKILL.md" >> "$WORKDIR/test_issues.md"
    else
        echo ""
...
| Task | Agent Config |
|------|--------------|
| Explore codebase for patterns | `Task(subagent_type: explore)` |
| Analyze test failures | `Task(subagent_type: general)` |
| Implement code fixes | `Task(subagent_type: general)` |
| Research architecture docs | `Task(subagent_type: explore)` |
...
- Log files in `logs/` provide full debugging context without bloating Agent's context
- This provides complete traceability from test results back to TDD session

## INVOKE
````
/3agents-tdd test/ui/job_definition_test.go
# → .opencode/workdir/2024-12-17-1430-tdd-job_definition/
...
- `.opencode/skills/frontend/SKILL.md` - Frontend changes
- `.opencode/skills/monitoring/SKILL.md` - UI tests
- `.opencode/skills/market-worker-test/SKILL.md` - Market worker tests (MANDATORY for `test/api/market_workers/`)

**Step 1.4: Read test architecture**
- `docs/TEST_ARCHITECTURE.md`

**Step 1.5: For market worker tests**
- Validate against skill: `.opencode/skills/market-worker-test/SKILL.md`
- Ensure required output files: `job_definition.json`, `schema.json`, `output.md`, `output.json`
