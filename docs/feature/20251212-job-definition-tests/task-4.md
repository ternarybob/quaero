# Task 4: Create nearby-restaurants-keywords job definition test

Workdir: ./docs/feature/20251212-job-definition-tests/ | Depends: 1 | Critical: no
Model: sonnet | Skill: go

## Context

This task is part of: Creating job definition test infrastructure for Quaero
Prior tasks completed: Task 1 - Framework helper methods added

## User Intent Addressed

Create test for nearby-restaurants-keywords job definition (multi-step: Places + Keywords) that runs end-to-end with monitoring and screenshots.

## Input State

Files that exist before this task:
- `test/ui/job_framework_test.go` - UITestContext with RunJobDefinitionTest method
- `test/config/job-definitions/nearby-restaurants-keywords.toml` - Multi-step job definition

## Output State

Files after this task completes:
- `test/ui/job_definition_nearby_restaurants_keywords_test.go` - Complete test file for multi-step job

## Skill Patterns to Apply

### From go/SKILL.md:
- **DO:** Skip tests when required environment not available
- **DO:** Wrap errors with context using %w
- **DO:** Put integration tests in test/ui/
- **DON'T:** Hard-code API keys

## Implementation Steps

1. Create `test/ui/job_definition_nearby_restaurants_keywords_test.go`
2. Define TestJobDefinitionNearbyRestaurantsKeywords function
3. Configure JobDefinitionTestConfig for nearby-restaurants-keywords:
   - JobName: "Nearby Restaurants + Keywords (Wheelers Hill)"
   - JobDefinitionPath: "../config/job-definitions/nearby-restaurants-keywords.toml"
   - Timeout: 8 minutes (places + agent keyword extraction)
   - RequiredEnvVars: ["QUAERO_GOOGLE_PLACES_API_KEY", "QUAERO_AGENT_GOOGLE_API_KEY"]
   - AllowFailure: true (agent step may hit rate limits)
4. Call RunJobDefinitionTest with config
5. Log success

## Code Specifications

```go
package ui

import (
    "testing"
    "time"
)

// TestJobDefinitionNearbyRestaurantsKeywords tests the multi-step Places + Keywords job
func TestJobDefinitionNearbyRestaurantsKeywords(t *testing.T) {
    utc := NewUITestContext(t, 15*time.Minute)
    defer utc.Cleanup()

    utc.Log("--- Testing Job Definition: Nearby Restaurants + Keywords ---")

    config := JobDefinitionTestConfig{
        JobName:           "Nearby Restaurants + Keywords (Wheelers Hill)",
        JobDefinitionPath: "../config/job-definitions/nearby-restaurants-keywords.toml",
        Timeout:           8 * time.Minute,
        RequiredEnvVars:   []string{"QUAERO_GOOGLE_PLACES_API_KEY", "QUAERO_AGENT_GOOGLE_API_KEY"},
        AllowFailure:      true, // Agent step may hit rate limits
    }

    if err := utc.RunJobDefinitionTest(config); err != nil {
        t.Fatalf("Job definition test failed: %v", err)
    }

    utc.Log("✓ Nearby Restaurants + Keywords job definition test completed successfully")
}
```

## Accept Criteria

- [ ] File `test/ui/job_definition_nearby_restaurants_keywords_test.go` exists
- [ ] Test function TestJobDefinitionNearbyRestaurantsKeywords defined
- [ ] Uses JobDefinitionTestConfig with correct values
- [ ] Requires both QUAERO_GOOGLE_PLACES_API_KEY and QUAERO_AGENT_GOOGLE_API_KEY
- [ ] AllowFailure set to true (rate limits possible)
- [ ] Timeout set to 8 minutes
- [ ] Code compiles: `go build ./test/ui/...`

## Handoff

After completion, next task(s): 6 (verification)
