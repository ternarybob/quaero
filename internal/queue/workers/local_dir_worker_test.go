...
func TestLocalDirWorker_HelperFunctions(t *testing.T) {
	t.Run("getStringConfig", func(t *testing.T) {
		...
		if got := getStringConfig(config, "key1", "default"); got != "value1" {
			t.Errorf("getStringConfig() = %v, want value1", got)
		}
		if got := getStringConfig(config, "key2", "default"); got != "default" {
			t.Errorf("getStringConfig() = %v, want default", got)
		}
		if got := getStringConfig(config, "missing", "default"); got != "default" {
			t.Errorf("getStringConfig() = %v, want default", got)
		}
	})
	...
	t.Run("getIntConfig", func(t *testing.T) {
		...
		if got := getIntConfig(config, "key1", 0); got != 42 {
			t.Errorf("getIntConfig() = %v, want 42", got)
		}
		if got := getIntConfig(config, "key2", 0); got != 123 {
			t.Errorf("getIntConfig() = %v, want 123", got)
		}
		if got := getIntConfig(config, "key3", 99); got != 99 {
			t.Errorf("getIntConfig() = %v, want 99", got)
		}
		if got := getIntConfig(config, "missing", 99); got != 99 {
			t.Errorf("getIntConfig() = %v, want 99", got)
		}
	})
	...
	t.Run("getStringSliceConfig", func(t *testing.T) {
		...
		got := getStringSliceConfig(config, "key1", nil)
		if len(got) != 3 || got[0] != "a" {
			t.Errorf("getStringSliceConfig() = %v, want [a b c]", got)
		}
		got = getStringSliceConfig(config, "key2", nil)
		if len(got) != 2 || got[0] != "x" {
			t.Errorf("getStringSliceConfig() = %v, want [x y]", got)
		}
		got = getStringSliceConfig(config, "key3", []string{"default"})
		if len(got) != 1 || got[0] != "default" {
			t.Errorf("getStringSliceConfig() = %v, want [default]", got)
		}
	})
}
