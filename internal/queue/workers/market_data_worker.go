// -----------------------------------------------------------------------
// MarketDataWorker - Fetches market price data for any exchange
...
// MarketDataWorker fetches market price data and calculates technical indicators.
...
var _ interfaces.DefinitionWorker = (*MarketDataWorker)(nil)
...
// MarketOHLCV represents a single day's price data
type MarketOHLCV struct {
	Date   time.Time
	Open   float64
	High   float64
	Low    float64
...
func (w *MarketDataWorker) GetType() models.WorkerType {
...
func (w *MarketDataWorker) ReturnsChildJobs() bool {
	return false
}
...
func (w *MarketDataWorker) fetchMarketData(ctx context.Context, ticker common.Ticker, period string) (*MarketData, error) {
	...
	// Convert to MarketOHLCV
	for _, d := range eodData {
		marketData.HistoricalPrices = append(marketData.HistoricalPrices, MarketOHLCV{
			Date:   d.Date,
			Open:   d.Open,
			High:   d.High,
			Low:    d.Low,
...
func (w *MarketDataWorker) calculateMarketTechnicals(data *MarketData) {
...
func (w *MarketDataWorker) calculateSMA(prices []float64, period int) float64 {
...
func (w *MarketDataWorker) calculateRSI(prices []float64, period int) float64 {
...
func (w *MarketDataWorker) findMin(values []float64) float64 {
	if len(values) == 0 {
		return 0
	}
	min := math.MaxFloat64
...
func (w *MarketDataWorker) findMax(values []float64) float64 {
	if len(values) == 0 {
		return 0
	}
	max := values[0]
...
func (w *MarketDataWorker) determineTrend(price, sma20, sma50, sma200, rsi float64) string {
	bullishSignals := 0
	bearishSignals := 0
...
func (w *MarketDataWorker) createMarketDocument(ctx context.Context, data *MarketData, jobDef *models.JobDefinition, parentJobID string, outputTags []string) *models.Document {
