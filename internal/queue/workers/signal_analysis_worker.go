// -----------------------------------------------------------------------
// SignalAnalysisWorker - Analyzes announcement signal quality
...
// SignalAnalysisWorker analyzes announcements for signal quality.
...
type SignalAnalysisWorker struct {
	*BaseMarketWorker
...
// Compile-time assertion: SignalAnalysisWorker implements DefinitionWorker
var _ interfaces.DefinitionWorker = (*SignalAnalysisWorker)(nil)

// NewSignalAnalysisWorker creates a new signal analysis worker.
func NewSignalAnalysisWorker(
	documentStorage interfaces.DocumentStorage,
	searchService interfaces.SearchService,
	...
	logger arbor.ILogger,
	jobMgr *queue.Manager,
) *SignalAnalysisWorker {
	return &SignalAnalysisWorker{
		BaseMarketWorker: NewBaseMarketWorker(
...
func (w *SignalAnalysisWorker) GetType() models.WorkerType {
	return models.WorkerTypeSignalAnalysis
}

// ReturnsChildJobs returns false - this worker executes inline.
func (w *SignalAnalysisWorker) ReturnsChildJobs() bool {
	return false
}

// ValidateConfig validates the worker configuration.
func (w *SignalAnalysisWorker) ValidateConfig(step models.JobStep) error {
	if step.Config == nil {
...
func (w *SignalAnalysisWorker) Init(ctx context.Context, step models.JobStep, jobDef models.JobDefinition) (*interfaces.WorkerInitResult, error) {
...
func (w *SignalAnalysisWorker) CreateJobs(ctx context.Context, step models.JobStep, jobDef models.JobDefinition, stepID string, initResult *interfaces.WorkerInitResult) (string, error) {
	if initResult == nil {
		var err error
...
func (w *SignalAnalysisWorker) processAnalysis(ctx context.Context, ticker common.Ticker, stepID string, outputTags []string) (*DocumentInfo, error) {
	...
	// 9. Generate tags
	tags := GenerateMarketTags(ticker.String(), "signal_analysis", now)
...
func (w *SignalAnalysisWorker) getAnnouncementsData(ctx context.Context, ticker string) ([]map[string]interface{}, string, time.Time, error) {
...
func (w *SignalAnalysisWorker) getPriceData(ctx context.Context, ticker string) ([]OHLCV, string, time.Time, error) {
	tickerLower := strings.ToLower(ticker)
...
func (w *SignalAnalysisWorker) classifyAnnouncement(ann map[string]interface{}, prices []OHLCV) (AnnouncementClassification, error) {
...
func (w *SignalAnalysisWorker) calculateMetrics(annDate time.Time, prices []OHLCV) ClassificationMetrics {
	metrics := ClassificationMetrics{}

	// Find prices by date
	priceByDate := make(map[string]OHLCV)
...
func (w *SignalAnalysisWorker) generateMarkdownSummary(schema *SignalAnalysisSchema) string {
...
func (w *SignalAnalysisWorker) collectTickers(config map[string]interface{}) []common.Ticker {
