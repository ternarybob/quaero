...
func (d *DocumentStorage) SearchByIdentifier(identifier string, excludeSources []string, limit int) ([]*models.Document, error) {
	...
	query := fmt.Sprintf(`
		...
		WHERE (
			...
			-- Search in content_markdown (case-insensitive)
			OR LOWER(content_markdown) LIKE '%%' || LOWER(?) || '%%'
		)
		%s
...
func (d *DocumentStorage) GetStats() (*models.DocumentStats, error) {
	...
	d.db.db.QueryRow("SELECT AVG(LENGTH(content_markdown)) FROM documents").Scan(&avgSize)
	if avgSize.Valid {
