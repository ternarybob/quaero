...
func (w *OrchestratorWorker) CreateJobs(ctx context.Context, step models.JobStep, jobDef models.JobDefinition, stepID string, initResult *interfaces.WorkerInitResult) (string, error) {
	...
	// Get the pipeline manager ID for document isolation
	// This ensures documents created by child tools are tagged with the pipeline's manager ID,
	// enabling downstream steps (like format_output) to find them
	pipelineManagerID := workerutil.GetManagerID(ctx, w.jobMgr, stepID)
	...
	for wave := 0; wave < maxWaves; wave++ {
		...
		for _, planStep := range waveSteps {
			...
			jobPayload := map[string]interface{}{
				...
				"manager_id":    pipelineManagerID, // Link to pipeline manager for document isolation
				"step_name":     step.Name,
				"original_goal": goal,
			}
