...
// GitWorker handles GitHub repository cloning via git command
...
// Compile-time assertions: GitWorker implements both interfaces
var _ interfaces.DefinitionWorker = (*GitWorker)(nil)
var _ interfaces.JobWorker = (*GitWorker)(nil)

// NewGitWorker creates a new GitHub git worker
func NewGitWorker(
	connectorService interfaces.ConnectorService,
	jobManager *queue.Manager,
	queueMgr interfaces.QueueManager,
...
func (w *GitWorker) Execute(ctx context.Context, job *models.QueueJob) error {
...
func (w *GitWorker) GetType() models.WorkerType {
...
func (w *GitWorker) Init(ctx context.Context, step models.JobStep, jobDef models.JobDefinition) (*interfaces.WorkerInitResult, error) {
	...
	// Extract config
	connectorID := workerutil.GetStringConfig(stepConfig, "connector_id", "")
	connectorName := workerutil.GetStringConfig(stepConfig, "connector_name", "")
	owner := workerutil.GetStringConfig(stepConfig, "owner", "")
	repo := workerutil.GetStringConfig(stepConfig, "repo", "")
	triggerURL := workerutil.GetStringConfig(stepConfig, "trigger_url", "")
	...
	if (owner == "" || repo == "") && triggerURL != "" {
		...
		if extractedBranch != "" && workerutil.GetStringConfig(stepConfig, "branch", "") == "" {
	...
	branch := workerutil.GetStringConfig(stepConfig, "branch", "main")
	extensions := workerutil.GetStringSliceConfig(stepConfig, "extensions", []string{".go", ".ts", ".tsx", ".js", ".jsx", ".md", ".yaml", ".yml", ".toml", ".json"})
	excludePaths := workerutil.GetStringSliceConfig(stepConfig, "exclude_paths", []string{"vendor/", "node_modules/", ".git/", "dist/", "build/"})
	maxFiles := workerutil.GetIntConfig(stepConfig, "max_files", 1000)
	...
	gitPath := workerutil.GetStringConfig(stepConfig, "git_path", defaultGitPath)
...
func (w *GitWorker) CreateJobs(ctx context.Context, step models.JobStep, jobDef models.JobDefinition, stepID string, initResult *interfaces.WorkerInitResult) (string, error) {
...
func (w *GitWorker) ReturnsChildJobs() bool {
	return true
}

// ValidateConfig validates step configuration for GitHub git type
func (w *GitWorker) ValidateConfig(step models.JobStep) error {
	if step.Config == nil {
...
func (w *GitWorker) logDocumentSaved(ctx context.Context, job *models.QueueJob, docID, title, path string) {
...
func (w *GitWorker) parseGitHubURL(githubURL string) (owner, repo, branch string) {
