// -----------------------------------------------------------------------
// NewsWorker - Fetches company announcements for any exchange
// Uses EODHD News API for non-ASX exchanges.
// For ASX tickers, delegates to AnnouncementsWorker (Markit API has better ASX coverage).
// -----------------------------------------------------------------------
...
var _ interfaces.DefinitionWorker = (*NewsWorker)(nil)
...
// NewNewsWorker creates a new market news worker
func NewNewsWorker(
	documentStorage interfaces.DocumentStorage,
	kvStorage interfaces.KeyValueStorage,
	logger arbor.ILogger,
...
func (w *NewsWorker) getEODHDAPIKey(ctx context.Context) string {
	if w.kvStorage == nil {
...
func (w *NewsWorker) GetType() models.WorkerType {
	return models.WorkerTypeMarketNews
}

// Init initializes the announcements worker
func (w *NewsWorker) Init(ctx context.Context, step models.JobStep, jobDef models.JobDefinition) (*interfaces.WorkerInitResult, error) {
	...
	if ticker.Exchange == "ASX" && w.announcementsWorker != nil {
		w.logger.Info().
			Str("ticker", ticker.String()).
			Msg("Delegating to ASX announcements worker")
		return w.announcementsWorker.Init(ctx, step, jobDef)
	}
...
func (w *NewsWorker) CreateJobs(ctx context.Context, step models.JobStep, jobDef models.JobDefinition, stepID string, initResult *interfaces.WorkerInitResult) (string, error) {
	if initResult == nil {
		var err error
	...
	if ticker.Exchange == "ASX" && w.announcementsWorker != nil {
		...
		return w.announcementsWorker.CreateJobs(ctx, step, jobDef, stepID, nil)
	}
...
func (w *NewsWorker) ReturnsChildJobs() bool {
	return false
}

// ValidateConfig validates step configuration
func (w *NewsWorker) ValidateConfig(step models.JobStep) error {
	if step.Config == nil {
...
func (w *NewsWorker) fetchAnnouncements(ctx context.Context, ticker common.Ticker, period string, limit int) ([]Announcement, error) {
...
func (w *NewsWorker) createAnnouncementsSummaryDoc(
	ctx context.Context,
