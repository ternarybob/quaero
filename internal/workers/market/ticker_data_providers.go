...
// FundamentalsProvider implements FundamentalsDataProvider using FundamentalsWorker.
...
// PriceDataProvider implements PriceDataProvider using DataWorker.
// It first checks for cached documents, then generates fresh data if needed.
...
func NewPriceProvider(
	...
	worker := NewDataWorker(documentStorage, kvStorage, logger, nil)
...
// AnnouncementsProvider implements AnnouncementDataProvider using AnnouncementsWorker.
// It first checks for cached documents, then generates fresh data if needed.
type AnnouncementsProvider struct {
	documentStorage interfaces.DocumentStorage
	kvStorage       interfaces.KeyValueStorage
	logger          arbor.ILogger
	worker          *AnnouncementsWorker
	cacheHours      int
}
...
// SetCacheHours sets the cache duration in hours.
func (p *AnnouncementsProvider) SetCacheHours(hours int) {
	p.cacheHours = hours
}

// GetAnnouncements retrieves announcement data for a ticker.
// Returns cached data if fresh, generates new data if missing/stale.
func (p *AnnouncementsProvider) GetAnnouncements(ctx context.Context, ticker string, limit int) (*interfaces.TickerDataResult, error) {
	parsed := common.ParseTicker(ticker)
	if parsed.Code == "" {
		return nil, fmt.Errorf("invalid ticker: %s", ticker)
	}

	sourceType := models.WorkerTypeMarketAnnouncements.String()
	sourceID := parsed.SourceID("market_announcements")
	...
	// Generate fresh data using worker's CreateJobs
	p.logger.Info().
		Str("ticker", parsed.String()).
		Int("limit", limit).
		Msg("Generating fresh announcements data")

	// Create a synthetic step for the worker
	step := models.JobStep{
		Name: "announcements-data-provider",
		Type: models.WorkerTypeMarketAnnouncements,
		Config: map[string]interface{}{
			"ticker": parsed.String(),
			"limit":  limit,
			"period": "Y1", // Default to 1 year
		},
	}
	jobDef := models.JobDefinition{
		ID:   "ticker-data-provider",
...
// isFresh checks if a document is still fresh based on cache hours.
func (p *AnnouncementsProvider) isFresh(doc *models.Document) bool {
	if doc == nil || doc.LastSynced == nil {
		return false
	}
	return time.Since(*doc.LastSynced) < time.Duration(p.cacheHours)*time.Hour
...
// getDocumentDate extracts the document date from metadata.
func (p *AnnouncementsProvider) getDocumentDate(doc *models.Document) time.Time {
	if doc.LastSynced != nil {
		return *doc.LastSynced
	}
	return doc.CreatedAt
