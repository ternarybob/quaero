// job_definition_smsf_portfolio_test.go - SMSF Portfolio Review UI tests
// Tests the SMSF portfolio job_template job that spawns child jobs for each stock.
//
// Key assertions:
// 1. Child jobs (stock analyses) appear in queue immediately when parent starts
...
// TestSMSFPortfolioChildJobVisibility tests that child jobs appear in queue
// when the parent SMSF portfolio job starts, not only after completion.
// This validates UI reactivity for job_template workers.
func TestSMSFPortfolioChildJobVisibility(t *testing.T) {
	utc := NewUITestContext(t, MaxJobTestTimeout)
	defer utc.Cleanup()

	utc.Log("--- Testing SMSF Portfolio Review: Child Job Visibility ---")
	utc.Log("Stock to analyze: ROC (AI)")

	// Expected stock tickers that should appear as child jobs
	expectedTickers := []string{"ROC"}

	jobName := "SMSF Aggressive Portfolio Review (Test)"
	...
	utc.Screenshot("smsf_portfolio_before_trigger")
	...
	for {
		if time.Since(startTime) > jobTimeout {
			utc.Screenshot("smsf_portfolio_timeout")
			break
		}
		...
		if time.Since(lastScreenshotTime) >= screenshotInterval {
			...
			utc.Screenshot(fmt.Sprintf("smsf_portfolio_queue_%ds", elapsed))
			lastScreenshotTime = time.Now()
		}
		...
		// Get child jobs (jobs with "ASX Stock Review" or "ASX:" in name)
			...
			chromedp.Evaluate(`
				(() => {
					...
					return component.allJobs
						.filter(j => j.name && (j.name.includes('ASX Stock Review') || (j.name.includes('ASX:') && !j.name.includes('SMSF'))))
		...
		if parentStatus == "completed" || parentStatus == "failed" {
			...
			utc.Screenshot("smsf_portfolio_parent_" + parentStatus)
	...
	// Final screenshot
	utc.Screenshot("smsf_portfolio_final_state")
...
// TestSMSFPortfolioCompletion tests the full SMSF portfolio job flow
// This test focuses on successful completion rather than reactivity
func TestSMSFPortfolioCompletion(t *testing.T) {
	utc := NewUITestContext(t, MaxJobTestTimeout)
	defer utc.Cleanup()

	utc.Log("--- Testing SMSF Portfolio Review: Full Completion ---")
	...
	utc.Screenshot("smsf_portfolio_completion_before")
	...
	utc.Screenshot("smsf_portfolio_completion_final")
	utc.Log("Test completed")
}
